...
export default async function html(pHtml, pIo, pNode) {
  ...
  try {
    ...
    lHtml = lHtml.replace(
      /<article><\/article>/g,
      // `<article v-if="!data">${lHtm}</article><article v-else v-html="data"></article>`
      `<article v-if="!data">${lHtm}</article><article v-else><v-runtime-template :parent="this" :template="data"></v-runtime-template></article>`
    );
    let lUrl = "";
    if (pNode.url) {
