...
const initColor = computed(() => {
  const cmpClass = fromClassesOnce("text", mask, false);
...
const variant = computed({
  /** @returns {string} значение цвета текста */
  get() {
    return fromClassesOnce("text", mask, false);
  },
  /** @param {string} value значение цвета текста */
  set(value) {
    deleteClasses("text", mask, false);
