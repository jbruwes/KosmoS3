...
export default async function html(pHtml, pIo, pNode) {
  ...
  try {
    ...
    lHtml = lHtml.replace(
      /<article><\/article>/g,
      // `<article v-if="!template">${lHtm}</article><article v-else v-html="template"></article>`
      `<article v-if="!template">${lHtm}</article><article v-else><v-runtime-template :parent="this" :template="template"></v-runtime-template></article>`
    );
    let lUrl = "";
    if (pNode.url) {
